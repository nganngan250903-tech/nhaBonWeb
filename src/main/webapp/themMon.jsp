<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Thêm món</title>

<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="<%=request.getContextPath()%>/CSS/admin.css">
</head>

<body class="bg-light">

<!-- SIDEBAR -->
<jsp:include page="layout/adminSidebar.jsp"/>

<!-- NỘI DUNG CHÍNH -->
<div class="admin-content">

    <h3>➕ Thêm món</h3>

    <form action="QLMonController" method="post" class="w-50">
        <input type="hidden" name="action" value="insert">

        <input class="form-control mb-2" name="tenMon" placeholder="Tên món">
        <input class="form-control mb-2" name="gia" placeholder="Giá">
        <input class="form-control mb-2" name="hinhAnh" placeholder="Link ảnh">
        <textarea class="form-control mb-2" name="moTa" placeholder="Mô tả"></textarea>
        <input class="form-control mb-2" name="maDM" placeholder="Mã danh mục">
        <input class="form-control mb-2" name="soLuong" placeholder="Số lượng">

        <select class="form-select mb-3" name="trangThai">
            <option value="1">BestSeller</option>
            <option value="0"></option>
        </select>

        <button class="btn btn-success">Lưu</button>
    </form>

</div>

</body>
</html>
